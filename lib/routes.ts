/**
 * Route manifest and pure helpers. No server imports.
 *
 * Split out of lib/seo.ts so client components can read it. The language
 * switcher in the navigation needs the per-locale slug map: without it, it
 * swapped the locale prefix and kept the path, which 404s on every FR only
 * page and on every route whose slug differs per locale.
 *
 * lib/seo.ts re-exports all of this, so existing "@/lib/seo" imports are
 * unaffected.
 */
/**
